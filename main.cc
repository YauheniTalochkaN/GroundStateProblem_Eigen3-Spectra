...
{       
    for (int64_t state = 0; state < dim; ++state) 
    {
        for (int64_t i = 0; i < N; ++i) 
        {
            ...
            {
                H1x.coeffRef(flipped_state, state) += complexType(0.5, 0.0);

                H1y.coeffRef(flipped_state, state) += complexType(0.0, 0.5) * sign_i;
            } */
...
{    
    ...
    Eigen::SparseMatrix<complexType, Eigen::ColMajor, int64_t> H1z_colm(DIM, DIM);
    Eigen::SparseMatrix<complexType, Eigen::ColMajor, int64_t> H_colm(DIM, DIM);

    build_sparse_hamiltonian(N, DIM, Jedges, H1z_colm, H_colm);

    H1z_colm.makeCompressed();
    H_colm.makeCompressed();

    Eigen::SparseMatrix<complexType, Eigen::RowMajor, int64_t> H1z(H1z_colm);

    H1z_colm.data().squeeze();
    H1z_colm.resize(0, 0); 

    Eigen::SparseMatrix<complexType, Eigen::RowMajor, int64_t> H(H_colm);
    ...
    for(size_t iter = 0; iter < num_iters; ++iter)
    {
        ...
        Spectra::SparseHermMatProd<complexType, Eigen::Lower, Eigen::RowMajor, int64_t> opH(H);

        Spectra::HermEigsSolver<Spectra::SparseHermMatProd<complexType, Eigen::Lower, Eigen::RowMajor, int64_t>> eigsH(opH, 1, 10);
